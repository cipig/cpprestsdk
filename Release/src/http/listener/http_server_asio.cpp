...
will_deref_and_erase_t asio_server_connection::async_write(WriteFunc response_func_ptr, const http_response& response)
{
    if (m_ssl_stream)
    {
        asio::async_write(*m_ssl_stream, m_response_buf, [= CPPREST_CAPTURE_THIS](const std::error_code& ec, std::size_t) {
            (this->*response_func_ptr)(response, ec);
        });
    }
    else
    {
        asio::async_write(*m_socket, m_response_buf, [= CPPREST_CAPTURE_THIS](const std::error_code& ec, std::size_t) {
            (this->*response_func_ptr)(response, ec);
        });
    }
    return will_deref_and_erase_t {};
